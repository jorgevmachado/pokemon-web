...
export async function GET(request: Request): Promise<NextResponse<TPaginatedListResponse<TMyPokemon> | {
  message: string
}>> {
  ...
  try {
    ...
    const listQuery: MyPokemonListQuery = {
      limit ,
      page ,
      type ,
      nickname ,
    ...
    const response = await service.list(listQuery) as TPaginatedListResponse<TMyPokemon>;
